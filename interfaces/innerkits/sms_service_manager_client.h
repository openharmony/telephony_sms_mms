...
public:
     ...
     * @return int32_t, returns {@code 0} if success.
     */
    int32_t SetCBConfigList(int32_t slotId, const std::vector<int32_t>& messageIds, int32_t ranType);
     ...
     * ranging from {@code 0} to the maximum card slot index number supported by the device.
     * @param desAddr [in], indicates the destination address.
     * @return int32_t, returns {@code -1} if the SMS destination address is an unknown short code type,
     * returns {@code 0} if the SMS destination address is not a premium short code type,
     * returns {@code 1} if the SMS destination address is a possible premium short code type.
     */
