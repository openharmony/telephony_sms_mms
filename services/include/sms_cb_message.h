...
class SmsCbMessage {
public:
    static constexpr uint8_t SMS_CB_IOS639LANG_SIZE = 3;
    /**
    ...
    struct SmsCbMessageHeader {
        ...
        {
            return serialNum.geoScope == other.serialNum.geoScope &&
                serialNum.msgCode == other.serialNum.msgCode && msgId == other.msgId;
        }
    };
    ...
    bool GetFormat(int8_t &format) const;
    bool GetPriority(int8_t &priority) const;
    bool GetGeoScope(uint8_t &gs) const;
    bool GetSerialNum(uint16_t &serial) const;
    bool GetServiceCategory(uint16_t &categoty) const;
    bool IsEtwsPrimary(bool &isPriority) const;
    bool IsEtwsMessage(bool &isEtws) const;
    bool IsCmasMessage(bool &isCmas) const;
    bool IsEtwsEmergencyUserAlert(bool &isUserAlert) const;
    bool IsEtwsPopupAlert(bool &isPopupAlert) const;
    bool GetWarningType(uint16_t &warnType) const;
    bool GetCmasSeverity(int8_t &severity) const;
    bool GetCmasUrgency(int8_t &urgency) const;
    bool GetCmasCertainty(int8_t &certainty) const;
    bool GetCmasCategory(int8_t &cmasCategory) const;
    bool GetCmasResponseType(int8_t &cmasRes) const;
    bool GetMessageId(uint16_t &msgId) const;
    bool GetCmasMessageClass(int8_t &cmasClass) const;
...
private:
    ...
    static constexpr uint8_t SMS_CB_HEADER_SIZE = 6;
    static constexpr uint8_t SMS_CBMI_LIST_SIZE_MAX = 50; // Maximum SMS CBMI list size
    static constexpr uint16_t MAX_CB_MSG_TEXT_LEN = 4200;
    static constexpr uint8_t MAX_CBMSG_PAGE_SIZE = 82;
    static constexpr uint16_t MAX_CBMSG_TOTAL_PAGE_SIZE = 1252;
    static constexpr uint16_t MAX_CBMSG_PAGE_BEAR_SIZE = MAX_CBMSG_TOTAL_PAGE_SIZE * 8 / 7;
    static constexpr uint8_t MAX_ETWS_SIZE = 56;
    static constexpr uint8_t MAX_CBMSG_PAGE_NUM = 15;

    static constexpr uint8_t SMS_DCS_ALPHABET_GROUP = 0x01;
    static constexpr uint16_t SMS_ETWS_BASE_MASK = 0x1100;
