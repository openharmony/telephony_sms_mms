    ...
    SmsCodingScheme CodingScheme, struct SmsUserData *pUserData)
{
    ...
    switch (CodingScheme) {
        case SMS_CODING_7BIT:
            ...
            break;
        case SMS_CODING_8BIT:
            decodeSize = Decode8bitData(pTpdu, tpduLen, bHeaderInd, pUserData, nullptr);
            break;
        case SMS_CODING_UCS2:
    ...
    SmsCodingScheme CodingScheme, struct SmsUserData *pUserData, struct SmsTpud *pTPUD)
{
    ...
    switch (CodingScheme) {
        case SMS_CODING_7BIT:
            ...
            break;
        case SMS_CODING_8BIT:
            decodeSize = Decode8bitData(pTpdu, tpduLen, bHeaderInd, pUserData, pTPUD);
            break;
        case SMS_CODING_UCS2:
...
int GsmSmsUDataCodec::EncodeGSMData(const struct SmsUserData *pUserData, char *pEncodeData)
{
    ...
    for (int i = 0; i < pUserData->headerCnt; i++) {
        int headerLen = EncodeHeader(pUserData->header[i], &(pEncodeData[offset]));
    ...
    packSize = SmsCommonUtils::Pack7bitChar(reinterpret_cast<const unsigned char *>(pUserData->data), pUserData->length,
        fillBits, reinterpret_cast<unsigned char *>(&pEncodeData[offset]));
    ...
    struct SmsUserData *pUserData, struct SmsTpud *pTPUD)
{
    ...
    if (bHeaderInd == true) {
        ...
        for (int i = 0; offset < udhl && i < MAX_UD_HEADER_NUM; i++) {
            int headerLen = DecodeHeader(&(pTpdu[offset]), tpduLen, &(pUserData->header[i]));
            if (headerLen <= 0) {
    ...
    TELEPHONY_LOGI("fillBits = %{public}d udhl = %{public}d udl = %{public}d offset = %{public}d", fillBits, udhl,
        udl, offset);
    pUserData->length = SmsCommonUtils::Unpack7bitChar(
        &(pTpdu[offset]), udl, fillBits, reinterpret_cast<unsigned char *>(pUserData->data), MAX_USER_DATA_LEN + 1);
    ...
    const unsigned char *pTpdu, int tpduLen, bool bHeaderInd, struct SmsUserData *pUserData, struct SmsTpud *pTPUD)
{
    ...
    if (bHeaderInd == true) {
        ...
        pUserData->headerCnt = 0;
        for (int i = 0; offset < udhl && i < MAX_UD_HEADER_NUM; i++) {
            if (!GetHeaderCnt(pTpdu, tpduLen, pUserData, offset, udhl, i)) {
                return 0;
            }
        }
    } else {
...
int GsmSmsUDataCodec::DecodeHeader(const unsigned char *pTpdu, int pduLen, struct SmsUDH *pHeader)
{
    int offset = 0;
    ...
    switch (pHeader->udhType) {
        ...
        case SMS_UDH_ALTERNATE_REPLY_ADDRESS: {
            ...
            offset += GsmSmsParamCodec::DecodeAddress(&pTpdu[offset], pduLen, &(pHeader->udh.alternateAddress));
