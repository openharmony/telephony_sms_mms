...
public:
    ...
    void TextBasedSmsDelivery(const std::string &desAddr, const std::string &scAddr, const std::string &text,
        ...
        const sptr<IDeliveryShortMessageCallback> &deliveryCallback, int32_t id, bool isMmsApp = true) override;
    void TextBasedSmsDeliveryViaIms(const std::string &desAddr, const std::string &scAddr, const std::string &text,
        const sptr<ISendShortMessageCallback> &sendCallback,
        const sptr<IDeliveryShortMessageCallback> &deliveryCallback, int32_t dataBaseId, bool isMmsApp = true);
    ...
    void SendSmsForEveryIndexer(int &i, std::vector<struct SplitInfo> cellsInfos, const std::string &desAddr,
        ...
        int32_t dataBaseId, bool isMmsApp = true);
    ...
    void UpdateIndexerInfo(std::shared_ptr<SmsSendIndexer> &smsIndexer, uint8_t msgRef8bit,
        ...
        long timeStamp, uint16_t msgId, int32_t dataBaseId, bool isMmsApp);
