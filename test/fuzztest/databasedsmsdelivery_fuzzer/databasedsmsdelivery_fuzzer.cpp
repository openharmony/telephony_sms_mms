...
#include "databasedsmsdelivery_fuzzer.h"
...
void DoSomethingInterestingWithMyAPI(const uint8_t *data, size_t size)
{
    ...
    std::uint16_t port = static_cast<uint16_t>(size);
    ...
    if (smsSendManager->gsmSmsSender_ != nullptr) {
        smsSendManager->gsmSmsSender_->DataBasedSmsDelivery(
            desAddr, scAddr, port, data, size, sendCallback, deliveryCallback);
    }
    if (smsSendManager->cdmaSmsSender_ != nullptr) {
        smsSendManager->cdmaSmsSender_->DataBasedSmsDelivery(
            desAddr, scAddr, port, data, size, sendCallback, deliveryCallback);
    }
    return;
}
