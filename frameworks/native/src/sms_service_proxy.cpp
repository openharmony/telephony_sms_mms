...
std::vector<int32_t> SmsServiceProxy::CalculateLength(const std::u16string &message, bool force7BitCode)
{
    ...
    remote->SendRequest(CALCULATE_LENGTH, dataParcel, replyParcel, option);
    int32_t resultLen = replyParcel.ReadInt32();
    for (int32_t i = 0; i < resultLen; ++i) {
        result.emplace_back(replyParcel.ReadInt32());
    }
    return result;
}
