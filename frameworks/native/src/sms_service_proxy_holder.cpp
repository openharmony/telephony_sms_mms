    ...
    const sptr<IDeliveryShortMessageCallback> &deliveryCallback)
{
    if (InitSmsServiceProxy()) {
        ...
        TELEPHONY_LOGI("execd smsServiceInterface_->SendMessage\n");
        return ERROR_NONE;
    }
    return ERROR_SERVICE_UNAVAILABLE;
}
...
std::vector<int32_t> SmsServiceProxyHolder::CalculateLength(const std::u16string &message, bool force7BitCode)
{
    if (InitSmsServiceProxy()) {
        return smsServiceInterface_->CalculateLength(message, force7BitCode);
    }
    std::vector<std::int32_t> defaultValue;
    return defaultValue;
}
} // namespace Telephony
} // namespace OHOS